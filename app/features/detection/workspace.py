"""Detection workspace: per-object bounding-box annotation.

Three columns — Classes + Objects-in-image (left) | Images (middle: browse
imported images) | Canvas (right: draw/move/resize/delete boxes on the
selected image, zoomable). One image can hold objects of many different
classes, so there's no "active class" driving what a new box gets tagged
with — drawing a box immediately opens a search-as-you-type class picker
anchored on it instead, and right-clicking an existing box offers the same
picker to change its class, or delete it. Images are never copied or
...
from app.shared.autolabel import detection as autolabel_detection
...
_ACCENT = TASK_ACCENTS["detection"]
_GRID_ZOOM_STEP = 24

_INSTRUCTIONS = (
    "1.  Click and drag on the image to draw a box around an object.\n"
    "2.  Pick its class from the search box that appears — type to filter, or click straight from the list.\n"
    "3.  Click an existing box to select it — drag its body to move it, drag a corner/edge to resize.\n"
    "4.  Hold Shift and drag to draw a new box on top of or inside an existing one.\n"
    "5.  Right-click a box to change its class or delete it, or press Delete on a selected box."
)

_AUTOLABEL_DESCRIPTION = (
    "Runs a trained object detector across your images and draws a box for every object it finds above "
    "the confidence threshold. Expects a YOLOv5/v8/v11-style ONNX export (box coordinates plus per-class "
    "scores, optionally with a separate objectness score) — the two most common ONNX detector layouts."
)
_AUTOLABEL_DUPLICATE_WARNING = "Running on images that already have boxes adds more on top — it won't replace or deduplicate the existing ones."


class DetectionWorkspace(QWidget):
    def __init__(self, controller: ProjectController) -> None:
        ...
        self.canvas = DetectionCanvas(controller)
        ...
        export_button.setToolTip("Export labeled images as a YOLO, Pascal VOC, or CreateML dataset")
    ...
    def _on_selection_changed(self, image_ids: list[str]) -> None:
        # Grid selection can include many images (multi-select), but the
        # canvas only ever edits one; fall back to the current/focused one.
    ...
    def _build_autolabel_task(self) -> AutoLabelTask:
        return AutoLabelTask(
            ...
            default_input_size=autolabel_detection.DEFAULT_SIZE,
            build_predictor=autolabel_detection.build_predictor,
            apply_predictions=autolabel_detection.apply_predictions,
            needs_iou=True,
    ...
    def _on_classes_changed(self) -> None:
        ...
        # A class rename/removal can change colors/labels or invalidate
