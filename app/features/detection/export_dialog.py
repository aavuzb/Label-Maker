"""Dialog for exporting a detection project to YOLO / Pascal VOC / CreateML."""
...
from app.features.detection.export import FORMAT_LABELS, FORMAT_YOLO, export_dataset
from app.features.project.manager import ProjectController
from app.shared.labeling.panel_widgets import build_callout
from app.shared.theme import FS_CAPTION, TASK_ACCENTS, primary_button_style

_ACCENT = TASK_ACCENTS["detection"]

_FORMAT_HELP = {
    "yolo": (
        "images/{train,val,test}/ + labels/{train,val,test}/ of matching .txt files, "
        "plus classes.yaml — the layout YOLO training expects."
    ),
    "voc": (
        "JPEGImages/ + Annotations/ (one .xml per image) + ImageSets/Main/{train,val,test}.txt "
        "— the classic Pascal VOC layout."
    ),
    "createml": (
        "One folder per split, each holding its images plus a single annotations.json "
        "— the layout Apple's Create ML object detector expects."
    ),
}
...
class ExportDialog(QDialog):
    def __init__(self, controller: ProjectController, parent=None) -> None:
        ...
        self.help_label = build_callout(_FORMAT_HELP[FORMAT_YOLO], _ACCENT)
        ...
        self.buttons.button(QDialogButtonBox.Ok).setStyleSheet(primary_button_style(_ACCENT))
    ...
    def _do_export(self) -> None:
        ...
        if result.exported_count == 0:
            QMessageBox.warning(self, "Export Dataset", "Nothing was exported — no images have bounding boxes yet.")
            return
        ...
        if result.skipped_unannotated:
            message += f"\n\nSkipped {result.skipped_unannotated} image(s) with no boxes."
