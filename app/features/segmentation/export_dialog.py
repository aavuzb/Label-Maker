"""Dialog for exporting a segmentation project to YOLO-seg / COCO."""
...
from app.features.segmentation.export import FORMAT_LABELS, FORMAT_YOLO, export_dataset
...
_FORMAT_HELP = {
    "yolo": (
        "images/{train,val,test}/ + labels/{train,val,test}/ of matching .txt files (one polygon per "
        "line: class_id x1 y1 x2 y2 ...), plus classes.yaml — the layout YOLO segmentation training expects."
    ),
    "coco": (
        "{train,val,test}/ image folders plus annotations/instances_{split}.json holding COCO-style "
        "polygon segmentations — widely supported by segmentation training tools (Detectron2, MMDetection, "
        "pycocotools-based pipelines)."
    ),
}
...
class ExportDialog(QDialog):
    ...
    def _do_export(self) -> None:
        ...
        if result.exported_count == 0:
            QMessageBox.warning(self, "Export Dataset", "Nothing was exported — no images have polygons yet.")
            return
        ...
        if result.skipped_unannotated:
            message += f"\n\nSkipped {result.skipped_unannotated} image(s) with no polygons."
